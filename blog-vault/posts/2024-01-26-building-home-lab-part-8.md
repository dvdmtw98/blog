...
![banner-image|640](building-home-lab-part-8-banner.png)
...
![vbox-36|500](vbox-36.png)

However, VirtualBox lets you add up to 8 interfaces to a VM. To use more than 4 interfaces the VirtualBox CLI has to be used.

### VirtualBox CLI Setup

The VirtualBox CLI binary is called **`VBoxManage.exe`**.

![vbox-37|500](vbox-37.png)
...
![vbox-42|400](vbox-42.png)

### Creating New Interface

You can interact with VMs from the CLI using the VM name or the VM GUID (Both are visible in the image above). I will be using the VM name to make the required changes.

To be able to add a network interface the VM has to be in the "Power Off" state.

![vbox-43|580](vbox-43.png)
...
# Use the "Intel Pro Desktop" Adapter
...
![vbox-45|440](vbox-45.png)

> [!NOTE]
> Interfaces that are created using the CLI will not show up in the Settings page. To modify the adapter settings the CLI will have to be used.
...
![pfsense-83|500](pfsense-83.png)

Enter **`1`** to select "Assign Interfaces".  
Should VLANs be set up now? **`n`**

![pfsense-84|500](pfsense-84.png)

Enter the WAN interface name: **`em0`**  
Enter the LAN interface name: **`em1`**  
Enter the Optional 1 interface name: **`em2`**  
Enter the Optional 2 interface name: **`em3`**  
Enter the Optional 3 interface name: **`em4`**

![pfsense-85|500](pfsense-85.png)

Do you want to proceed?: **`y`**

![pfsense-86|500](pfsense-86.png)

The new interface has been added. Now we need to assign the interface an IP address.

![pfsense-87|500](pfsense-87.png)

### Interface Naming & IP Assignment

In Part 2, I had shown how the IP address assignment and DHCP configuration can be performed using the CLI. For this interface I will perform the same actions but using the GUI.

From the Kali VM access the pfSense GUI. Go to **`Interfaces -> OPT3`**.

![pfsense-88|500](pfsense-88.png)

In the <u>Description</u> field enter **`ISOLATED`**.  
Make sure <u>IPv4 Configuration Type</u> is set to **`Static IPv4`**.  
Ensure that <u>IPv6 Configuration Type</u> is set to **`None`**.

![pfsense-89|500](pfsense-89.png)

Scroll to the <u>Static IPv4 Configuration</u> section.  
In the <u>IP Address</u> enter **`10.99.99.1`**** and set the subnet to **`24`**.

Scroll to the bottom and click on <u>Save</u>.

![pfsense-90|500](pfsense-90.png)

Click on **`Apply Changes`** in the popup that appears to persist the changes.  
Now interface OPT3 (ISOLATED) will have an IP address.

![pfsense-91|500](pfsense-91.png)

You can go to the pfSense VM and press **`Enter`** to refresh the screen and confirm that the interface has been assigned a name and IP address.

![pfsense-94|560](pfsense-94.png)

### Interface DHCP Configuration

From the navigation bar select **`Services -> DHCP Server`**. 

![pfsense-92|560](pfsense-92.png)

Select the **`ISOLATED`** interface.  
Select the <u>Enable DHCP server on ISOLATED interface</u> option.  

In the <u>Primary Address Pool</u> section provide the DHCP Pool range. I have selected **`10.99.99.100`** as the start address and **`10.99.99.199`** as the end address.  

Scroll to the bottom of the page and click on <u>Save</u>.  

![pfsense-93|560](pfsense-93.png)
...
![pfsense-97|560](pfsense-97.png)

In the popup click on **`Apply Changes`** to persist the new rule.

![pfsense-98|560](pfsense-98.png)

![pfsense-99|560](pfsense-99.png)
...
![pfsense-118|600](pfsense-118.png)

Click on **`Submit`**.

![pfsense-119|600](pfsense-119.png)
...
![vbox-71|540](vbox-71.png)

Install Windows as shown below. The installation process is identical to the process from the previous module (Part 7).

![user-1|540](user-1.png)

![user-2|540](user-2.png)

![user-4|540](user-4.png)

![user-5|540](user-5.png)

![user-6|540](user-6.png)

![user-7|540](user-7.png)

Select "Domain join instead".

![user-10|540](user-10.png)
...
![flare-13|540](flare-13.png)

![user-13|540](user-13.png)

![user-14|540](user-14.png)

![user-15|540](user-15.png)
...
![user-17|500](user-17.png)

![user-18|380](user-18.png)

![user-19|540](user-19.png)

![user-20|400](user-20.png)

![user-22|400](user-22.png)

![user-23|400](user-23.png)

After rebooting the VM. Remove the Guest Addition Image.

![user-25|400](user-25.png)
...
![flare-21|540](flare-21.png)

Click on **`Take`**.

![flare-30|460](flare-30.png)

Give the Snapshot a descriptive name. Click on **`OK`** to create the Snapshot.

![flare-22|400](flare-22.png)
...
![flare-46|400](flare-46.png)

![flare-47|540](flare-47.png)
...
![remnux-5|540](remnux-5.png)

### Post-Install Configuration

Select the VM and from the toolbar click on **`Start`**.

![remnux-6|540](remnux-6.png)
...
![remnux-8|540](remnux-8.png)

Once the update is complete restart the VM.

![remnux-9|260](remnux-9.png)

![remnux-10|380](remnux-10.png)
...
![remnux-11|540](remnux-11.png)

#### Creating VM Snapshot

Click on the <u>Hamburger menu</u> and select **`Snapshot`**.

![remnux-17|400](remnux-17.png)

Click on **`Take`** to create a Snapshot.

![remnux-18|540](remnux-18.png)

Give the Snapshot a descriptive name. Then click on **`OK`**.

![remnux-12|420](remnux-12.png)

![remnux-13|540](remnux-13.png)
...
[Part 9 - Tsurugi Linux (DFIR) Setup](https://blog.davidvarghese.com/posts/building-home-lab-part-9/)
