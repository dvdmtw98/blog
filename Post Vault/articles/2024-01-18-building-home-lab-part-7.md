...
In the previous module, we installed Windows Server 2019, installed AD Domain Services, configured DHCP and set up a DNS Forwarder. In this module, we will continue building out the AD Lab by creating AD users and adding devices to the AD environment.
...
Open the <u>Start menu</u> and click on "<u>Windows Administrative Tools</u>" then choose **`Group Policy Management`**.
...
Expand the sidebar folders to the following: **`Computer Configuration -> Policies -> Administrative Templates -> Network -> Network Connections -> Windows Defender Firewall -> Domain Profile`**. Select "<u>Windows Defender Firewall: Protect all network connections</u>". Click on **`Edit policy settings`**.
...
Now whenever a new device joins our AD environment the Group Policies that are configured will automatically applied to them. With this, we have completed the Domain Controller setup.
...
> [!INFO]
> Snapshots can be taken with the VM in a running state but sometimes doing so can cause unexpected problem. So I would recommend "Powering off" all the VMs before taking a Snapshot. 
...
Many other features and services can enabled on the DC. Refer to the following links for variations on the installation process.
